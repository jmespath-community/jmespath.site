...
```code
search(max_by(people, &age), null) -> {"age":50,"age_str":"50","bool":false,"name":"d"}
```

```code
search(max_by(people, &age), null) -> 50
```

```code
search(max_by(people, &to_number(age_str)), null) -> {"age":50,"age_str":"50","bool":false,"name":"d"}
```

```code
search(max_by(people, &age_str), null) -> ERROR: invalid-type
```

```code
search(max_by(people, age), null) -> ERROR: invalid-type
```
